...
z2 = X * Theta1';
% m x size(Theta1, 1): for each sample, we have a corresponding value
% for the hidden layer input
...
% Theta matrices for regularization
...
% Regularize
JReg = sum(sum(Theta1_reg .^ 2)) + sum(sum(Theta2_reg .^ 2));
...
a2_t = a2';
delta3 = H - y;
delta2 = (Theta2' * delta3) .* a2_t .* (1 - a2_t);
delta2 = delta2(2 : end, :);
